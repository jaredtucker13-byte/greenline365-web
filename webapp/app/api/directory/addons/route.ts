...
const ADDON_CATALOG: Record<string, { name: string; price: number; mode: 'subscription' | 'payment'; interval?: 'week' | 'month'; description: string }> = {
    coupon_engine: {
          ...
          mode: 'subscription',
          interval: 'month',
          description: 'Create trackable QR coupons for your listing'
    },
    custom_poll: {
          name: 'Custom Poll Template',
          price: 150,
          mode: 'payment',
          description: 'Industry-specific feedback poll template'
    },
    featured_boost: {
          name: 'Featured Boost',
          price: 29,
          mode: 'payment',
          description: '1-week homepage spotlight for your listing'
    },
    additional_photos: {
          name: 'Additional Photos (5-pack)',
          price: 9,
          mode: 'subscription',
          interval: 'month',
          description: '5 extra photos beyond your tier limit'
    },
    analytics_pro: {
          name: 'Analytics Pro',
          price: 19,
          mode: 'subscription',
          interval: 'month',
          description: 'Detailed views, clicks, and search analytics'
    },
};
    ...
    try {
      ...
      if (listing.tier === 'free') {
              return NextResponse.json(
                { error: 'Marketplace add-ons require a Pro or Premium subscription' },
                { status: 403 }
                      );
      }
