...
export async function POST(req: NextRequest) {
  try {
    ...
    const { data, error } = await supabase
      ...
      .insert([{
        full_name: full_name,
        company: sanitizeInput(body.company) || null,
        role: sanitizeInput(body.role) || null,
        business_name: sanitizeInput(body.business_name) || null,
        website: sanitizeInput(body.website) || null,
        industry: sanitizeInput(body.industry) || null,
        needs: body.needs ? (Array.isArray(body.needs) ? body.needs : [sanitizeInput(body.needs)]) : null,
        notes: sanitizeInput(body.notes) || null,
        preferred_datetime: preferred_datetime,
        alternate_datetime: body.alternate_datetime || null,
        email: email,
        phone: sanitizeInput(body.phone) || null,
