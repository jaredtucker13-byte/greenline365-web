          ...
          <section id="categories" className="max-w-7xl mx-auto px-6 py-16" data-testid="categories-section">
            ...
            <div className="grid grid-cols-2 md:grid-cols-4 gap-3 md:gap-4">
              ...
              {[CATEGORIES[0], CATEGORIES[1]].map(cat => (
                <div
                  key={cat.id}
                  className="relative rounded-2xl overflow-hidden cursor-pointer group hover:shadow-[0_0_24px_rgba(255,140,0,0.10)] transition-shadow duration-500"
                  style={{ minHeight: 160 }}
                  onClick={() => handleCategoryClick(cat.id)}
                ...
                <div className="grid grid-cols-2 gap-4">
                  {[
                    ...
                    <div key={f.label} className="flex items-center gap-3 group/feat cursor-default">
                      <div className="w-10 h-10 rounded-lg flex items-center justify-center flex-shrink-0 group-hover/feat:scale-110 transition-transform duration-300" style={{ background: 'rgba(255,140,0,0.1)' }}>
                        <svg className="w-5 h-5" style={{ color: '#FF8C00' }} fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={1.5}>
                          <path strokeLinecap="round" strokeLinejoin="round" d={f.icon} />
                        </svg>
                      </div>
                      <span className="text-sm font-medium text-[#1a1a1a] group-hover/feat:text-orange-600 transition-colors duration-300">{f.label}</span>
                    </div>
                  ))}
                </div>
