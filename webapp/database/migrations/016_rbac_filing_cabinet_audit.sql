...
-- Roles: owner, manager, tech, guest
DO $$
BEGIN
  IF NOT EXISTS (
...
-- 2. Team Members Table (for inviting staff)
-- ============================================
...
CREATE TABLE IF NOT EXISTS filing_cabinet (
  ...
  file_type TEXT NOT NULL, -- 'receipt', 'warranty', 'contract', 'invoice', 'tax_doc', 'photo', 'other'
  ...
  subcategory TEXT, -- e.g., 'materials', 'gas', 'labor'
  
  -- Financial data (for receipts/invoices)
  amount DECIMAL(10,2),
  currency TEXT DEFAULT 'USD',
  tax_year INTEGER,
  job_id TEXT, -- Link to a specific job if applicable
  ...
  -- AI processing
  ai_extracted_data JSONB DEFAULT '{}'::jsonb, -- OCR data, categorization
...
CREATE TABLE IF NOT EXISTS audit_logs (
  ...
  -- What
  action TEXT NOT NULL, -- 'create', 'read', 'update', 'delete', 'upload', 'download', 'export', 'login', 'impersonate'
  entity_type TEXT NOT NULL, -- 'filing_cabinet', 'property', 'contact', 'team_member', 'settings', etc.
  ...
  old_value JSONB,
  new_value JSONB,
  metadata JSONB DEFAULT '{}'::jsonb, -- IP address, user agent, etc.
...
-- Team Members: Users can see team members in their tenant
CREATE POLICY IF NOT EXISTS "team_members_tenant_read" ON team_members
  FOR SELECT USING (
    tenant_id IN (
...
-- Team Members: Only owners can manage team
CREATE POLICY IF NOT EXISTS "team_members_owner_write" ON team_members
  FOR ALL USING (
    tenant_id IN (
...
-- Filing Cabinet: INSERT - Anyone in the tenant can upload
CREATE POLICY IF NOT EXISTS "filing_cabinet_tenant_insert" ON filing_cabinet
...
-- Filing Cabinet: SELECT - Only owner and manager roles can view
-- Techs can only see files they uploaded (within 5 minutes, for confirmation)
CREATE POLICY IF NOT EXISTS "filing_cabinet_role_read" ON filing_cabinet
  FOR SELECT USING (
    tenant_id IN (
      SELECT business_id FROM user_businesses WHERE user_id = auth.uid()
    )
    AND (
      -- Owners and managers can see everything
      EXISTS (
      ...
      OR
      -- Techs can only see their own recent uploads (5 min window)
      (
        uploaded_by = auth.uid() 
        AND created_at > NOW() - INTERVAL '5 minutes'
      )
      OR
      -- Owner of the business always has access
      EXISTS (
...
-- Filing Cabinet: DELETE - Only owners (soft delete tracked in audit)
CREATE POLICY IF NOT EXISTS "filing_cabinet_owner_delete" ON filing_cabinet
  FOR DELETE USING (
    tenant_id IN (
      SELECT business_id FROM user_businesses 
...
-- Audit Logs: Read - Owners and managers only
CREATE POLICY IF NOT EXISTS "audit_logs_read" ON audit_logs
  FOR SELECT USING (
    tenant_id IN (
      SELECT business_id FROM user_businesses WHERE user_id = auth.uid()
    )
...
-- Audit Logs: Insert - System can always write (via service role)
...
-- 7. Property Health Score Function
-- ============================================
...
BEGIN
  FOR v_asset IN 
    SELECT * FROM assets WHERE property_id = p_property_id
  LOOP
    -- Deduct points based on asset age
    IF v_asset.install_date IS NOT NULL THEN
      ...
      END IF;
    ELSE
      v_score := v_score - 10; -- Unknown install date
    END IF;
  ...
  FROM interactions WHERE property_id = p_property_id;
  
  IF v_last_service_days IS NULL THEN
    v_score := v_score - 15;
  ELSIF v_last_service_days > 365 THEN
    v_score := v_score - 10;
  ELSIF v_last_service_days > 180 THEN
    v_score := v_score - 5;
  END IF;
